...
// Computation runs in a web worker; main thread only builds DOM.
  ...
  function buildHostsAccountsCard(scope, s) {
    ...
    if (scope.devicesSorted.length) {
      ...
      card.appendChild(ovScrollList(scope.devicesSorted, ([device, count]) =>
        ovRow(s.deviceCol, device, device, count.toLocaleString())
      ));
    }
    ...
    if (scope.usersSorted.length) {
      ...
      card.appendChild(hdr);
      card.appendChild(ovScrollList(scope.usersSorted, ([user, count]) =>
        ovRow(s.userCol, user, user, count.toLocaleString())
      ));
    }
  ...
  function buildActivityCard(activity, s) {
    ...
    const title = document.createElement('div'); title.className = 'ov-card-title'; title.textContent = 'Activity';
    card.appendChild(title);
  ...
  function buildProcessCard(process, s) {
    ...
    title.appendChild(sub); card.appendChild(title);
    card.appendChild(ovScrollList(process.sorted, ([name, count]) => ovRow(s.fileCol, name, name, count.toLocaleString())));
    return card;
  }
  ...
  function buildNetworkCard(network, s) {
    ...
    if (network.ipSorted && network.ipSorted.length) {
      ...
      card.appendChild(hdr);
      card.appendChild(ovScrollList(network.ipSorted, ([ip, count]) => ovRow(s.remoteIpCol, ip, ip, count.toLocaleString())));
